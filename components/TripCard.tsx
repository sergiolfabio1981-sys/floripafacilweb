...
const TripCard: React.FC<TripCardProps> = ({ trip: item }) => {
  ...
  const price = (item as any).price || (item as any).pricePerNight || (item as any).totalPrice || (item as any).pricePerDay || 0;
  ...
  return (
    <Link to={linkUrl} className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 flex flex-col border border-gray-100 hover:-translate-y-2">
      ...
      <div className="p-6 flex-grow flex flex-col">
        ...
        <h3 className="text-lg font-bold text-gray-800 line-clamp-1 group-hover:text-green-600 transition-colors">{title}</h3>
        ...
        {isCar && (
            <div className="flex gap-3 mb-4">
                <span className="text-[10px] bg-slate-100 px-2 py-0.5 rounded font-bold">⚙️ {(item as any).transmission}</span>
                <span className="text-[10px] bg-slate-100 px-2 py-0.5 rounded font-bold">👥 {(item as any).passengers}</span>
            </div>
        )}
